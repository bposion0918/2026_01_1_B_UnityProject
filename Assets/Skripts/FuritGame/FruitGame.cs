...
public class FruitGame : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (currentFruit != null)
        {
            ...
            float halfFruitSize = fruitSizes[currentFruitType] / 2.0f;
    ...
    void SpawnNewFruit()
    {
        if(!isGameOver)
        {
            ...
            Vector3 spwanPosition = new Vector3(worldPosition.x, fruitStartHeight, 0);

            float halfFruitSize = fruitSizes[currentFruitType] / 2.0f;

            spwanPosition.x = Mathf.Clamp(spwanPosition.x, -gameWidth / 2 + halfFruitSize, gameWidth / 2 - halfFruitSize);

            currentFruit = Instantiate(fruitPrefabs[currentFruitType], spwanPosition, Quaternion.identity);
